<?php
    ...
    if($changePassword == true) {
        $errorMessage = validateSignUpForm($username, $passwordSha256, $realname, $passport, $telephone, $region, $email);
    } else {
        $errorMessage = validateSignUpForm($username, 'thisisforvalidation', $realname, $passport, $telephone, $region, $email);
    }
    ...
    try {
        ...
        if($changePassword) {
            $updateInfoSTMT = $conn -> prepare("UPDATE customer SET password = ?, realname = ?, passportID = ?, telephone = ?, region = ?, email = ? WHERE username = ?");
            $updateInfoSTMT -> execute( array($passwordSha256, $realname, $passport, $telephone, $region, $email, $username));
        } else {
            $updateInfoSTMT = $conn -> prepare("UPDATE customer SET realname = ?, passportID = ?, telephone = ?, region = ?, email = ? WHERE username = ?");
            $updateInfoSTMT -> execute( array($realname, $passport, $telephone, $region, $email, $username));
        }
        $conn = NULL;
        updateSuccess();

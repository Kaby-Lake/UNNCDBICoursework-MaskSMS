<?php
    ...
    if($recaptcha == '') {
                signUpFailed('You have to pass the reCAPTCHA <i class="fa fa-check-square-o"></i> before sign up');
    }
    ...
    try {
        ...
        $sameUsernameSTMT = $conn -> prepare("SELECT customerID FROM customer WHERE username = ?");
        ...
        if (count($sameUsernameArray) != 0) {
            ...
            // add user
            $addUserSTMT = $conn -> prepare("INSERT INTO customer VALUES (null, ?, ?, ?, ?, ?, ?, ?)");
